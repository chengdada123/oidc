...
func randomHex(n int) string { buf := make([]byte, n); _, _ = rand.Read(buf); return hex.EncodeToString(buf) }
func generateClientID() string { return "bridge_" + randomHex(12) }
func generateClientSecret() string { return randomHex(32) }
func secretHint(secret string) string { if len(secret) <= 8 { return secret }; return secret[:8] }
...
func (s *Server) handleUpdateEmailLimit(w http.ResponseWriter, r *http.Request) {
	...
	if limit > 0 { _ = s.db.SetEmailLimitPerUser(limit) }
...
func (s *Server) handleCreateDomain(w http.ResponseWriter, r *http.Request) {
	...
	if targetID > 0 && domain != "" { _ = s.db.CreateDomain(targetID, domain, description, enabled) }
	http.Redirect(w, r, "/admin", http.StatusFound)
}
...
func (s *Server) handleUpdateDomain(w http.ResponseWriter, r *http.Request) {
	...
	description := strings.TrimSpace(r.FormValue("description"))
	enabled := r.FormValue("enabled") == "1"
	if id > 0 && targetID > 0 && domain != "" { _ = s.db.UpdateDomain(id, targetID, domain, description, enabled) }
	http.Redirect(w, r, "/admin", http.StatusFound)
}

func (s *Server) handleDeleteDomain(w http.ResponseWriter, r *http.Request) { id := parseInt64(chi.URLParam(r, "id")); if id > 0 { _ = s.db.DeleteDomain(id) }; http.Redirect(w, r, "/admin", http.StatusFound) }
...
func (s *Server) handleCreateTarget(w http.ResponseWriter, r *http.Request) {
	...
	if t.Name != "" && t.LoginURL != "" && t.RedirectURL != "" { _ = s.db.CreateTarget(t) }
	http.Redirect(w, r, "/admin", http.StatusFound)
}
...
func (s *Server) handleUpdateTarget(w http.ResponseWriter, r *http.Request) {
	...
	if clientID == "" && current != nil { clientID = current.ClientID }
	...
	if t.ID > 0 && t.Name != "" && t.LoginURL != "" && t.RedirectURL != "" { _ = s.db.UpdateTarget(t) }
	http.Redirect(w, r, "/admin", http.StatusFound)
}
...
func (s *Server) handleDeleteTarget(w http.ResponseWriter, r *http.Request) { id := parseInt64(chi.URLParam(r, "id")); if id > 0 { _ = s.db.DeleteTarget(id) }; http.Redirect(w, r, "/admin", http.StatusFound) }
...
func (s *Server) handleAdminPage(w http.ResponseWriter, r *http.Request) {
	...
	s.renderer.Render(w, "admin.html", AdminPageData{Title: "Admin", Domains: rows, Targets: targets, EmailLimit: limit})
}
