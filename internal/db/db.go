...
func Open(path string) (*Store, error) {
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil { return nil, fmt.Errorf("create db dir: %w", err) }
	db, err := sql.Open("sqlite", path)
	if err != nil { return nil, fmt.Errorf("open sqlite: %w", err) }
	if err := migrate(db); err != nil { _ = db.Close(); return nil, err }
...
func migrate(db *sql.DB) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, sub TEXT NOT NULL UNIQUE, email TEXT NOT NULL, name TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL);`,
	...
	for _, stmt := range stmts { if _, err := db.Exec(stmt); err != nil { return fmt.Errorf("migrate sqlite: %w", err) } }
	for _, stmt := range compat { _, _ = db.Exec(stmt) }
	return nil
}
